    ...
    useEffect(() => {
        ...
        socket.on('video', (data: { filename: string; path: string; metadata: any; timestamp: string }) => {
            ...
            fetch(videoUrl, { method: 'HEAD' })
                .then(res => {
                    if (!res.ok) {
                        console.error('[Hook] ❌ Video URL not accessible:', res.status);
                    } else {
                        console.log('[Hook] ✅ Video URL verified accessible');
                    }
                })
                .catch(err => console.error('[Hook] ❌ Video URL check failed:', err));
        ...
        socket.on('audio', (data: { filename: string; path: string; text: string; timestamp: string }) => {
            ...
            fetch(audioUrl, { method: 'HEAD' })
                .then(res => {
                    if (!res.ok) {
                        console.error('[Hook] ❌ Audio URL not accessible:', res.status);
                    } else {
                        console.log('[Hook] ✅ Audio URL verified accessible, Content-Type:', res.headers.get('content-type'));
                    }
                })
                .catch(err => console.error('[Hook] ❌ Audio URL check failed:', err));
